...
![](Regresja_files/figure-gfm/unnamed-chunk-1-1.png)<!-- -->
...
``` r
ggplot(q, aes(stations, mag))+
  geom_point(size=0.8)+
  ggtitle("Mag in different stations") +
  theme(
    plot.title=element_text( hjust=0.5, vjust=0.5, face='bold')
  )
```

![](Regresja_files/figure-gfm/unnamed-chunk-2-1.png)<!-- -->
...
![](Regresja_files/figure-gfm/unnamed-chunk-3-1.png)<!-- -->
...
``` r
scatterplot(stations~jitter(mag,factor=1, amount=0.5 ), data=q, pch=20, col=rgb(0.1,0.2,0.8, 0.3))
```

![](Regresja_files/figure-gfm/unnamed-chunk-4-1.png)<!-- -->

``` r
Quake.mod<-lm(stations~mag, data = q)
summary(Quake.mod)
```

    ## 
    ## Call:
    ## lm(formula = stations ~ mag, data = q)
    ## 
    ## Residuals:
...
``` r
ggplot(q, aes(stations, mag))+geom_point(size=3)+stat_smooth(method="lm", col="green")
```

    ## `geom_smooth()` using formula 'y ~ x'

![](Regresja_files/figure-gfm/unnamed-chunk-6-1.png)<!-- -->

``` r
plot(q$mag, q$stations, pch = 16)
abline(Quake.mod)
```

![](Regresja_files/figure-gfm/unnamed-chunk-7-1.png)<!-- --> \#zadanie 6
...
![](Regresja_files/figure-gfm/unnamed-chunk-8-1.png)<!-- -->

``` r
ggplot(q, aes(QuakeFittedValues, QuakeResiduals))+geom_point()+
  geom_hline(yintercept = 0, col="red", size=2)
```

![](Regresja_files/figure-gfm/unnamed-chunk-9-1.png)<!-- --> \#zadanie 7

``` r
hist(QuakeResiduals, prob=TRUE)
lines(density(QuakeResiduals, adjust=2.3))
```

![](Regresja_files/figure-gfm/unnamed-chunk-10-1.png)<!-- -->

``` r
#maja rozklad normalny
```

\#zadanie 8

``` r
c<-confint(Quake.mod, level=0.95)
c
```
...
``` r
plot(stations~jitter(mag,factor=1, amount=0.2 ), data=q, pch=20, cex=0.3, col="blue")
abline(Quake.mod, col="red")
abline(c[,1], col="green")
abline(c[,2], col="yellow")
```

![](Regresja_files/figure-gfm/unnamed-chunk-11-1.png)<!-- --> \#zadanie
9

``` r
#a
#RSS=summary(Quake.mod)$r.squared
RSS=sum(Quake.mod$residuals**2)
RSS
```
...
k=length(Quake.mod$coefficients)-1

#calculate total observations in dataset
n=length(Quake.mod$residuals)
...
``` r
FoV<-RSS/sum((q$stations-mean(q$stations))^2)
FoV
```
